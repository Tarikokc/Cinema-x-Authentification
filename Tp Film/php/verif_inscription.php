...
if (isset($_POST['submit'])) {
    ...
    $requetteidQ = $newPDO->prepare("SELECT id from questions WHERE libelle = '$libelle' ");
    $requetteidQ->execute();

    foreach ($requetteidQ->fetchAll(PDO::FETCH_ASSOC) AS $id){
        $idQ = $id['id'];
    ...
    $requeteUsers = $newPDO->prepare("INSERT INTO `users` (`id`, `idQ`, `nom`,`prenom`, `email`, `motdepasse`,`idPreference`) VALUES (NULL,'$idQ','$nom','$prenom', '$email', '$motdepasse','');)");
